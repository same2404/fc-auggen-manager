...
export const SetPieceHalfPitchEditor: React.FC<SetPieceHalfPitchEditorProps> = ({ players = [] }) => {
  ...
  const handleSaveAnimationToCollection = async () => {
    try {
      ...
      await setDoc(doc(db, 'animations', animDoc.id), animDoc);
      showToast('Animation erfolgreich in Collection "animations" gespeichert!');
    } catch (err) {
  ...
  const persistToStorageAndDb = async (updatedPieces: SetPieceVariant[]) => {
    ...
    if (active && db) {
      try {
        await setDoc(doc(db, 'set_pieces', active.id), active, { merge: true });
      } catch (err) {
