...
export const RunsSWView: React.FC<RunsSWViewProps> = ({
  ...
  const processPDFFile = async (file: File) => {
    ...
    setPdfToast(`PDF / Dokument wird per KI analysiert (${file.name})...`);
  ...
  return (
    <div 
      className="flex flex-col h-full bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden relative"
    ...
    >
      ...
      {/* Toast Notification */}
      {pdfToast && (
        <div className="fixed top-4 right-4 z-50 bg-slate-900 text-white px-4 py-2.5 rounded-xl border-2 border-amber-400 shadow-2xl flex items-center gap-3 animate-bounce">
      ...
      {/* Drag & Drop Overlay */}
      {dragActive && (
        <div className="absolute inset-0 z-40 bg-amber-400/90 backdrop-blur-xs border-4 border-dashed border-slate-900 flex flex-col items-center justify-center p-6 text-slate-900">
          ...
          <p className="text-xs font-bold uppercase mt-1">
            Die KI extrahiert alle Läufe, Zeiten und Ergebnisse automatisch für deinen Kader
          </p>
        </div>
      )}

      {/* Header Toolbar */}
      <div className="p-3 border-b-2 border-black bg-slate-100 flex flex-wrap justify-between items-center gap-3 shrink-0">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-slate-900 text-amber-400 rounded-lg border border-black shadow-sm">
            <Activity size={18} />
          </div>
          <div>
            <h3 className="font-black uppercase text-sm tracking-tight text-slate-900 leading-none">
              Läufe & Ausdauer (SW)
            </h3>
            <p className="text-[10px] font-bold text-slate-600 uppercase tracking-wider mt-0.5 flex items-center gap-1">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg border border-black font-black text-[10px] uppercase transition-all shadow-xs cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-amber-400 text-slate-950 px-3 py-1.5 rounded-lg border border-black hover:brightness-105 font-black text-[10px] uppercase transition-all shadow-xs cursor-pointer disabled:opacity-50"
            title="PDF, Foto oder Leistungsdiagnostik-Bericht hochladen & per KI importieren"
          >
            {isParsingPDF ? (
              <>
          ...
          <div className="relative flex items-center">
            ...
            <input 
              ...
              className="pl-8 pr-3 py-1.5 bg-white border border-slate-400 rounded-lg text-xs font-bold text-slate-900 focus:outline-none focus:border-black w-32 sm:w-40 placeholder:text-slate-400"
            />
            {searchTerm && (
              <button onClick={() => setSearchTerm('')} className="absolute right-2 text-slate-400 hover:text-slate-900">
                <X size={12} />
              </button>
            )}
          </div>
          ...
          {/* Category Filter */}
          <div className="flex items-center gap-1 bg-white border border-slate-400 rounded-lg p-0.5 text-[10px] font-bold">
            <Filter size={12} className="ml-1.5 text-slate-500" />
            <select 
              value={selectedCategory} 
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="bg-transparent text-slate-900 font-bold focus:outline-none py-1 pr-2 cursor-pointer"
            >
              ...
              <option value="medical">Sanitäter / Physio</option>
            </select>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-slate-900 text-white px-3 py-1.5 rounded-lg border border-black hover:bg-slate-800 text-[10px] font-black uppercase transition-all shadow-sm cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-red-700 text-white px-3 py-1.5 rounded-lg border border-black hover:bg-red-800 disabled:opacity-50 text-[10px] font-black uppercase transition-all shadow-sm cursor-pointer"
      ...
      <div 
        onClick={() => fileInputRef.current?.click()}
        className="bg-amber-50 hover:bg-amber-100 border-b-2 border-slate-900 px-4 py-2 flex items-center justify-between cursor-pointer group transition-colors shrink-0"
      >
        <div className="flex items-center gap-3">
          <div className="p-1.5 bg-amber-400 rounded border border-black text-slate-950 group-hover:scale-110 transition-transform">
            <Upload size={14} />
          </div>
          <div>
            <span className="text-xs font-black uppercase text-slate-900 flex items-center gap-1.5">
              <span>PDF-Laufbericht / Leistungsdiagnostik per KI importieren</span>
              <span className="bg-slate-900 text-amber-400 text-[8px] font-black px-1.5 py-0.5 rounded uppercase">
                KI Auto-Parsing
              </span>
            </span>
            <p className="text-[10px] font-bold text-slate-600">
              Lade ein PDF (z.B. Garmin, Polar, TRACKTICS, Excel-Liste oder Foto) hoch. Die KI trägt Zeiten & Ergebnisse automatisch den Spielern ein.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 text-slate-900 font-black text-xs uppercase group-hover:translate-x-1 transition-transform">
      ...
      <div className="flex-1 overflow-auto custom-scrollbar bg-slate-50 p-3 space-y-4">
        ...
        <div className="bg-white border-2 border-slate-900 rounded-xl overflow-hidden shadow-sm">
          ...
          <thead className="sticky top-0 bg-slate-900 text-white z-20 shadow-md">
            <tr>
              <th className="p-2 border border-slate-700 text-center w-10 align-top text-slate-300 font-black">Nr</th>
              <th className="p-2 border border-slate-700 text-left w-56 align-top text-white font-black">
                Spieler / Name
              </th>
              <th className="p-2 border border-slate-700 text-center w-16 align-top font-black text-amber-400">
                Pos
              </th>
              {displayedRunIndices.map((i) => {
                ...
                return (
                  <th key={i} className="p-2 border border-slate-700 text-center min-w-[130px] max-w-[160px] bg-slate-900 group">
                    <div className="flex flex-col gap-1.5">
                      <div className="flex items-center justify-between gap-1 border-b border-slate-700 pb-1">
                        <span className="text-[9px] font-black text-amber-400 uppercase">#{i + 1}</span>
                        {stats.count > 0 && (
                          <span className="text-[8px] bg-slate-800 text-slate-200 px-1 py-0.2 rounded font-mono">
                            {stats.count} Sp.
                          </span>
                        )}
                      </div>
                      ...
                      <input 
                        type="text"
                        className="bg-slate-800 text-white text-center focus:outline-none w-full text-[10px] font-black uppercase placeholder:text-slate-500 border border-slate-700 focus:border-amber-400 rounded px-1 py-0.5"
                        value={meta.name}
                      ...
                      <textarea
                        ...
                        className="bg-slate-800 text-slate-200 text-center focus:outline-none w-full text-[8px] font-bold placeholder:text-slate-500 resize-none h-9 custom-scrollbar border border-slate-700 focus:border-amber-400 rounded px-1 py-0.5"
                        value={meta.content || ''}
                      ...
                      {/* Run Date */}
                      <div className="flex items-center gap-1 justify-center bg-slate-800 border border-slate-700 rounded px-1 py-0.5">
                        <Calendar size={9} className="text-amber-400 shrink-0" />
                        <input 
                          type="date"
                          className="bg-transparent text-center focus:outline-none w-full text-[8px] font-bold text-slate-200 appearance-none cursor-pointer"
                          value={meta.date}
              ...
              return (
                <React.Fragment key={category}>
                  <tr className="bg-slate-200 border-y border-slate-400">
                    <td colSpan={3 + displayedRunIndices.length} className="p-2 font-black uppercase text-[10px] tracking-widest text-slate-900 bg-slate-200 border-l-4 border-l-slate-900">
                  ...
                  {categoryPlayers.map((player, pIdx) => {
                    ...
                    return (
                      <tr key={player.id} className="hover:bg-amber-50/60 transition-colors border-b border-slate-200 bg-white">
                        <td className="p-2 border-r border-slate-200 text-center font-mono font-bold text-slate-500 text-[10px]">
                          {pIdx + 1}
                        </td>
                        <td className="p-2 border-r border-slate-200">
                          <div className="flex items-center gap-2">
                            <span className="w-6 h-6 flex items-center justify-center bg-slate-900 text-white rounded border border-black text-[9px] font-black shrink-0 shadow-xs">
                        ...
                        <td className="p-2 border-r border-slate-200 text-center font-black text-red-700 uppercase text-[10px]">
                          {player.position || '-'}
                        </td>
                        {displayedRunIndices.map((i) => {
                          const val = record?.runs ? record.runs[i] || '' : '';
                          return (
                            <td key={i} className="p-0 border-r border-slate-200">
                              <input 
                                type="text"
                                className={`w-full h-full p-2.5 text-center bg-transparent font-black text-slate-900 text-xs focus:bg-amber-100 focus:outline-none transition-colors ${!isEditing ? 'cursor-not-allowed opacity-90' : ''}`}
                                value={val}
      ...
      {/* Footer Status Bar */}
      <div className="p-3 bg-slate-900 text-white border-t-2 border-black flex flex-wrap justify-between items-center gap-2 shrink-0">
      ...
      {showImportModal && parsedRunData && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border-2 border-black shadow-2xl rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
            
            {/* Modal Header */}
            <div className="p-4 bg-slate-900 text-white border-b-2 border-black flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-amber-400 text-slate-950 rounded-lg border border-black">
                  <Sparkles size={20} />
                </div>
                <div>
                  <h3 className="font-black uppercase text-sm tracking-wide flex items-center gap-2">
                    <span>KI PDF-Analyse Auswertung</span>
                    <span className="text-[9px] font-bold bg-slate-800 text-amber-400 px-2 py-0.5 rounded border border-slate-700">
                      {uploadFileName}
                    </span>
                  </h3>
            ...
            {/* Modal Body */}
            <div className="flex-1 overflow-auto p-6 bg-slate-50 space-y-6 custom-scrollbar">
              
              {/* Runs Identified Box */}
              <div>
                <h4 className="text-xs font-black uppercase text-slate-900 mb-2 flex items-center gap-1.5">
                  <Activity size={14} className="text-amber-500" />
                  <span>Erkannte Laufeinheiten ({parsedRunData.runs.length})</span>
                </h4>
                {parsedRunData.runs.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">Keine expliziten Laufbezeichnungen gefunden (Verwende Standard slots).</p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                    {parsedRunData.runs.map((r, idx) => (
                      <div key={idx} className="bg-white p-3 border border-slate-300 rounded-lg shadow-xs">
                        <div className="flex justify-between items-center text-[10px] font-black text-amber-600 mb-1">
                          <span>SLOT #{r.runIndex + 1}</span>
                          {r.runDate && <span>{r.runDate}</span>}
                        </div>
                        <p className="font-black text-slate-900 text-xs uppercase">{r.runName}</p>
                        {r.runContent && <p className="text-[10px] text-slate-600 mt-0.5">{r.runContent}</p>}
                      </div>
                    ))}
                  </div>
                )}
                      ...
                      {parsedRunData.playerResults.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-amber-50/50">
                            <td className="p-2 font-bold text-slate-900">{item.playerName}</td>
                            <td className="p-2">
                              {matchedPlayer ? (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-green-100 text-green-800 text-[10px] font-black uppercase">
                                  <Check size={10} />
                                  #{matchedPlayer.number || '?'} {matchedPlayer.lastName} {matchedPlayer.firstName}
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-amber-100 text-amber-800 text-[10px] font-black uppercase">
                                  <AlertCircle size={10} />
                            ...
                            <td className="p-2 text-center font-black text-amber-600">
                              {item.value}
                            </td>
                          </tr>
                        );
            ...
            {/* Modal Actions */}
            <div className="p-4 bg-slate-100 border-t-2 border-black flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setShowImportModal(false)}
                  className="px-4 py-2 bg-white border border-slate-400 hover:border-black rounded-lg text-xs font-bold text-slate-700 transition-colors cursor-pointer"
                >
                  Abbrechen
                </button>
                <button
                  onClick={handleConfirmImport}
                  className="px-5 py-2 bg-slate-900 text-amber-400 hover:bg-slate-800 rounded-lg border border-black text-xs font-black uppercase flex items-center gap-2 cursor-pointer shadow-sm"
                >
                  <Check size={16} />
      ...
      {showPrintModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border-2 border-black shadow-2xl rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-auto p-6 bg-slate-200 custom-scrollbar flex justify-center">
              <div className="bg-white border-2 border-slate-400 p-8 shadow-xl w-full max-w-4xl text-slate-900 space-y-6">
                    ...
                    <thead>
                      <tr className="bg-slate-900 text-white">
                        ...
                        {displayedRunIndices.map((i) => {
                          const meta = getRunMeta(i);
                          return (
                            <th key={i} className="border border-slate-900 p-2 text-center min-w-[80px]">
                        ...
                        return (
                          <tr key={player.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                            ...
                            {displayedRunIndices.map((i) => {
                              const val = record?.runs ? record.runs[i] || '-' : '-';
                              return (
                                <td key={i} className="border border-slate-300 p-1.5 text-center font-extrabold text-slate-900">
                                  {val}
                                </td>
                              );
                            })}
            ...
            <div className="p-4 bg-slate-100 border-t-2 border-black flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={handleExportPDF}
                  className="px-4 py-2 bg-red-700 text-white rounded-lg border border-black hover:bg-red-800 text-xs font-xs font-black uppercase flex items-center gap-2 cursor-pointer shadow-sm"
                >
                  <FileDown size={14} />
                ...
                <button
                  onClick={handlePrint}
                  className="px-4 py-2 bg-slate-900 text-white rounded-lg border border-black hover:bg-slate-800 text-xs font-black uppercase flex items-center gap-2 cursor-pointer shadow-sm"
                >
            ...
            <thead>
              <tr className="bg-slate-900 text-white">
                ...
                {displayedRunIndices.map((i) => {
                  const meta = getRunMeta(i);
                  return (
                    <th key={i} className="border border-slate-900 p-2 text-center">
                      ...
                      return (
                        <tr key={player.id} className={pIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                          ...
                          {displayedRunIndices.map((i) => {
                            const val = record?.runs ? record.runs[i] || '-' : '-';
                            return (
                              <td key={i} className="border border-slate-300 p-2 text-center font-extrabold text-slate-900">
                                {val}
                              </td>
                            );
                          })}
